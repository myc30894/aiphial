 ...
 * @author nickl
 */
public class PortedFilterMedOptimized extends DataArrayISAdapter
{

    private PrintStream LOG = null;
    final double EPSILON = 0.01;
    protected final int LIMIT = 100;
    final double TC_DIST_FACTOR = 0.5;
    ...
    protected void doFilter()
    {
        ...
        NewOptimizedFilter1(squareRange, (double) colorRange);


    }

    void NewOptimizedFilter1(double sigmaS, double sigmaR)
    {
        // Declare Variables
        ...
        for (i = 0; i < L; i++)
        {
            ...
            for (j = 0; j < pointCount; j++)
            {
                ...
                for (k = 0; k < N; k++)
                {
                    msRawData[N * modeCandidate_i + k] = (double) (yk[k + 2]);
                }
            }
            ...
            for (j = 0; j < N; j++)
            {
                msRawData[N * i + j] = (double) (yk[j + 2]);
            }

        // Prompt user on progress

        //    percent_complete = (double) (i / (double) (L)) * 100;
        //      msSys.Prompt("\r%2d%%", (int) (percent_complete + 0.5));


        // Check to see if the algorithm has been halted
       /*     if ((i % PROGRESS_RATE == 0) && ((ErrorStatus = msSys.Progress((double) (i / (double) (L)) * (double) (0.8)))) == ErrorLevel.EL_HALT)
        {
        break;
        }*/
        }
